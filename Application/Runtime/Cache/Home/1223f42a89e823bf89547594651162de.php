...
<link rel="stylesheet" type="text/css" href="/Public/Home/css/optstyle.css" />
		...
		<div class="clearfixLeft" id="clearcontent">
			...
			<div class="box">
				...
				<div class="tb-booth tb-pic tb-s310">
					<a href="<?php echo ($goods_pics[0]['pics_big']); ?>"><img src="<?php echo ($goods_pics[0]['pics_mid']); ?>" alt="细节展示放大镜特效" rel="<?php echo ($goods_pics[0]['pics_big']); ?>" class="jqzoom" /></a>
				</div>
				<ul class="tb-thumb" id="thumblist">
					<?php if(is_array($goods_pics)): $key = 0; $__LIST__ = $goods_pics;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pics): $mod = ($key % 2 );++$key;?><li <?php if($key == 1 ): ?>class="tb-selected"<?php endif; ?> >
						<div class="tb-pic tb-s40">
							<a href="javascript:void(0);"><img src="<?php echo ($pics["pics_sma"]); ?>" mid="<?php echo ($pics["pics_mid"]); ?>" big="<?php echo ($pics["pics_big"]); ?>"></a>
						</div>
					...
					<dd>
						...
						<div class="theme-popover">
							...
							<div class="theme-popbod dform">
								<form class="theme-signin" name="loginform" action="/index.php/Home/Cart/addcart" method="post">
									<input type="hidden" name="goods_id" value="<?php echo ($goods["id"]); ?>">
									<div class="theme-signin-left">
										<?php if(is_array($new_attr_radio)): $i = 0; $__LIST__ = $new_attr_radio;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol_radio): $mod = ($i % 2 );++$i;?><div class="theme-options">
												<div class="cart-title"><?php echo ($vol_radio[0]['attr_name']); ?></div>
												<ul>
													<?php if(is_array($vol_radio)): $key = 0; $__LIST__ = $vol_radio;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo_radio): $mod = ($key % 2 );++$key;?><li goods_attr_id="<?php echo ($vo_radio['id']); ?>" class="sku-line <?php if($key == 1 ): ?>selected<?php endif; ?>"><?php echo ($vo_radio['attr_value']); ?><i></i></li><?php endforeach; endif; else: echo "" ;endif; ?>
												</ul>
...
<div class="pay">
	...
	<li>
		<div class="clearfix tb-btn tb-btn-basket theme-login">
			<a id="LikBasket" title2="加入购物车" href="javascript:void(0);"><i></i>加入购物车</a>
		</div>
	</li>
</div>
			...
			<div class="am-tabs-bd">
				...
				<div class="am-tab-panel am-fade am-in am-active">
					<div class="J_Brand">
						...
						<div class="clear"></div>
						<ul id="J_AttrUL">
							<?php if(is_array($attr_only)): $i = 0; $__LIST__ = $attr_only;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol_only): $mod = ($i % 2 );++$i;?><li title=""><?php echo ($vol_only["attr_name"]); ?>:&nbsp;<?php echo ($vol_only["attr_value"]); ?></li><?php endforeach; endif; else: echo "" ;endif; ?>
						</ul>
						<div class="clear"></div>
					</div>
	...
	$(function(){
		$('#LikBasket').click(function(){
			...
			$.each(selected_li, function(i,v){
				//v代表单个li标签  document元素对象 需要先转化为jq对象
				// console.log($(v).attr('goods_attr_id'));
				goods_attr_ids += $(v).attr('goods_attr_id') +',';
			});
			//37,39,需要删除最后一个逗号 js中相关方法 substr  substring  slice 等
			//slice方法语法  stringObj.slice(起始位置从0开始,结束位置如果是负数表示倒数);
			goods_attr_ids = goods_attr_ids.slice(0,-1);

			// console.log(goods_attr_ids);return;
			//需要向表单加入goods_attr_ids数据  37,39
			//需要向表单加入一个隐藏域
			var str = "<input type='hidden' name='goods_attr_ids' value='" + goods_attr_ids + "'>";
			//表示把str这个隐藏域放到form标签内部
			$('form').append(str);
			//提交表单
			$('form').submit();
		});
